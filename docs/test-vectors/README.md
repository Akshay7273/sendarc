...
Deterministic, published vectors for the `sendarc/1` protocol and its crypto, so an
...
| `sendarc-crypto.json` | SendArc domain-specific KATs: invite-code → SPAKE2 scalar `w` (`HKDF "sendarc/1 spake2 w"`), the full SPAKE2 exchange with fixed secrets, `HKDF(Ke, "sendarc/1 master" \|\| TT)` master derivation, directional AEAD keys (`sendarc/1 o2j` / `j2o`), one AES-256-GCM seal vector (salt+counter nonce, header as AAD), and the SDP/ICE HMAC (`authmac`). | `packages/protocol/src/{spake2,keyschedule,aead,authmac}.test.ts`, `packages/wire/{spake2,authmac}_test.go` |
...
- The TypeScript side (`@sendarc/protocol`) asserts `rfc9382-p256.json` and
  `sendarc-crypto.json` against WebCrypto/noble-curves implementations (vitest).
...
reproducing. `sendarc-crypto.json` and `rfc9382-p256.json` were produced once by the
