...
| `files[].digestCheckpoint` (optional)    | Serialized whole-file digest state covering exactly `committedBlocks` (V13-PR05): `format` (e.g. `sha256-go-v1`, `sha256-wasm-v1`), the checkpointed `committedBlocks`/`committedBytes`, and the opaque hex `state`. Persisted atomically with the checkpoint; enables resume by state restore instead of re-hashing the persisted prefix. `null`/absent means resume re-hashes. |
...
- **Downgrade expectations:** a journal written by a newer build is not readable by an
  ...
  Within schema v1, `digestCheckpoint` is strictly additive and optional: a v1 journal
  without it (an older build's) decodes and resumes via re-hash, while a newer build's
  checkpoint can never be misread as data — a runtime that does not recognize the
  `format` restores nothing and re-hashes (correctness-first; the final whole-file
