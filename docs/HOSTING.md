# Self-hosting SendArc

SendArc ships as a single container that serves the web app, the signaling
...
**1. Terminate at `sendarcd`** — mount a PEM cert and key:

```sh
docker run -d --name sendarc \
  -p 8443:8443 \
...
- **STUN** — both clients use Google's STUN (`stun:stun.l.google.com:19302`)
  ...
  fixed at build time.
- **TURN** — SendArc does not speak TURN. When a direct peer-to-peer path
...
- `SENDARC_ALLOWED_ORIGINS` blocks cross-site socket abuse; keep it set
